...
static bool powNative(int argCount, Value* args) {
  if (argCount != 2 || !IS_NUMBER(args[0]) || !IS_NUMBER(args[1])) {
    args[-1] = OBJ_VAL(copyString("pow() expects 2 numbers.", 25));
    return false;
  }
  ...
  args[-1] = NUMBER_VAL(pow(base, exp));
  return true;
}

static bool factNative(int argCount, Value* args) {
  // Validate arguments
  if (argCount != 1 || !IS_NUMBER(args[0])) {
    args[-1] = OBJ_VAL(copyString("fact() expects 1 number.", 26));
    return false;
  }
  ...
  if (n < 0 || floor(n) != n) {
    args[-1] = OBJ_VAL(copyString("fact() expects a non-negative integer.", 43));
    return false;
  }
